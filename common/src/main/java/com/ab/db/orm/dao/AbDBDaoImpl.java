...
public class AbDBDaoImpl<T> extends AbBasicDBDao implements AbDBDao<T> {
	...
	public AbDBDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		LLog.d("clazz:" + this.clazz + " tableName:" + this.tableName + " idColumn:" + this.idColumn);
	}
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			LLog.d("[queryOne]: select * from " + this.tableName + " where "
					+ this.idColumn + " = '" + id + "'");
	...
	public List<T> rawQuery(String sql, String[] selectionArgs,Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			LLog.d("[rawQuery]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			getListFromCursor(clazz,list, cursor);
		} catch (Exception e) {
			LLog.e("[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			LLog.d("[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			if (cursor.getCount() > 0) {
				return true;
			}
		} catch (Exception e) {
			LLog.e("[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public List<T> queryList(String[] columns, String selection,
			...
			try {
				lock.lock();
				checkDBOpened();
				LLog.d("[queryList] from "+this.tableName+" where "+selection+"("+selectionArgs+")"+" group by "+groupBy+" having "+having+" order by "+orderBy+" limit "+limit);
				...
				LLog.e("[queryList] from DB Exception");
				e.printStackTrace();
			} finally {
				closeCursor(cursor);
				lock.unlock();
	...
	public long insert(T entity, boolean flag) {
			...
			try {
				...
				LLog.d("[insert]: insert into " + this.tableName + " " + sql);
				...
				for (Field relationsField : allFields) {
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							LLog.d("[insert]: insert into " + relationsTableName + " " + sql);
				...
				LLog.d("[insert] into DB Exception.");
				e.printStackTrace();
			}finally {
				lock.unlock();
			}
	 ...
	 * @see AbDBDao#insertList(java.util.List)
	 */
	@Override
	 ...
	 * @param flag the flag
	 * @return the long[] 插入成功的数据ID
	 * @see AbDBDao#insertList(java.util.List, boolean)
	 */
	@Override
	public long[] insertList(List<T> entityList,boolean flag) {
			...
			try {
				...
				for(int i=0;i<entityList.size();i++){
					...
					LLog.d("[insertList]: insert into " + this.tableName + " " + sql);
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							LLog.d("[insertList]: insert into " + relationsTableName + " " + sql);
							db.insert(relationsTableName, null, relationsCv);
						}
				...
				LLog.d("[insertList] into DB Exception.");
				e.printStackTrace();
			} finally {
				lock.unlock();
			}
	...
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			LLog.d("[delete]: delete from " + this.tableName + mLogSql);
	...
	public int deleteAll() {
		...
		try {
			lock.lock();
			checkDBOpened();
			LLog.d("[delete]: delete from " + this.tableName );
	 ...
	 * @param entityList the entity list
	 * @return the int 影响的行
	 * @see AbDBDao#updateList(java.util.List)
	 */
	@Override
	...
	public List<Map<String, String>> queryMapList(String sql,String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			LLog.d("[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
			...
			LLog.e("[queryMapList] from DB exception");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public int queryCount(String sql, String[] selectionArgs) {
       ...
       try{
    	   lock.lock();
    	   checkDBOpened();
    	   LLog.d("[queryCount]: " + getLogSql(sql, selectionArgs));
           cursor = db.query(this.tableName, null, sql, selectionArgs, null, null,null);
           if(cursor != null){
        	   count = cursor.getCount();
           }
       }catch (Exception e){
    	   LLog.e("[queryCount] from DB exception");
           e.printStackTrace();
       }finally{
    	   closeCursor(cursor);
    	   lock.unlock();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			lock.lock();
			checkDBOpened();
			LLog.d("[execSql]: " + getLogSql(sql, selectionArgs));
