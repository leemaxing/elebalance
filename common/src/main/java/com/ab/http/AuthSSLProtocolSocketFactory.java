 ...
 * with a {@link java.security.KeyStore truststore} file containg one or several trusted
 ...
 * with a {@link java.security.KeyStore keystore} file containg a private key/public
...
public class AuthSSLProtocolSocketFactory implements LayeredSocketFactory {
	...
	private static KeyManager[] createKeyManagers(final KeyStore keystore,
		...
		LLog.d("Initializing key manager");
	...
	private static TrustManager[] createTrustManagers(final KeyStore keystore)
		...
		LLog.d("Initializing trust manager");
		...
		try {
			...
			if (this.keystoreUrl != null) {
				...
				while (aliases.hasMoreElements()) {
					...
					if (certs != null) {
						LLog.d("Certificate chain '" + alias + "':");
						for (int c = 0; c < certs.length; c++) {
							if (certs[c] instanceof X509Certificate) {
								X509Certificate cert = (X509Certificate) certs[c];
								LLog.d(" Certificate " + (c + 1) + ":");
								LLog.d("  Subject DN: "
										+ cert.getSubjectDN());
								LLog.d("  Signature Algorithm: "
										+ cert.getSigAlgName());
								LLog.d("  Valid from: "
										+ cert.getNotBefore());
								LLog.d("  Valid until: "
										+ cert.getNotAfter());
								LLog.d("  Issuer: " + cert.getIssuerDN());
							}
						}
					}
				}
			...
			if (this.truststoreUrl != null) {
				...
				while (aliases.hasMoreElements()) {
					String alias = (String) aliases.nextElement();
					LLog.d("Trusted certificate '" + alias + "':");
					Certificate trustedcert = keystore
							.getCertificate(alias);
					if (trustedcert != null
							&& trustedcert instanceof X509Certificate) {
						X509Certificate cert = (X509Certificate) trustedcert;
						LLog.d("  Subject DN: " + cert.getSubjectDN());
						LLog.d("  Signature Algorithm: "
								+ cert.getSigAlgName());
						LLog.d("  Valid from: " + cert.getNotBefore());
						LLog.d("  Valid until: " + cert.getNotAfter());
						LLog.d("  Issuer: " + cert.getIssuerDN());
					}
				}
	 ...
	 * terminates and throws an {@link org.apache.http.conn.ConnectTimeoutException}
	 * </p>
	 * 
	 * @param host
	 *            the host name/IP
	 ...
	 * @param params
	 *            {@link org.apache.http.params.HttpConnectionParams Http connection parameters}
	 * 
