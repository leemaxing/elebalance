...
public class AbSlidingMenuView extends ViewGroup {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			LLog.d("--dispatchTouchEvent ACTION_DOWN--");
			...
			if (mTouchState == TOUCH_STATE_RESTART) {
				...
				LLog.d("--dispatchTouchEvent ACTION_DOWN return false--");
				return false;
			}
			break;
		case MotionEvent.ACTION_MOVE:
			LLog.d("--dispatchTouchEvent ACTION_MOVE--");
			mVelocityTracker.computeCurrentVelocity(1000,ViewConfiguration.getMaximumFlingVelocity());
			if (mScrollState == SCROLL_STATE_ALLOW && getWidth() - (int) ev.getX() < mWidth) {
				LLog.d("--dispatchTouchEvent ACTION_MOVE return true--");
				return true;
			}
			break;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		switch (ev.getAction()) {
		case MotionEvent.ACTION_DOWN:
			LLog.d("--onInterceptTouchEvent ACTION_DOWN--");
			mTouchState = mScroller.isFinished() ? TOUCH_STATE_RESTART : TOUCH_STATE_SCROLLING;
			if (mTouchState==TOUCH_STATE_SCROLLING) {
				LLog.d("--onInterceptTouchEvent ACTION_DOWN return false--");
				return false;
			}
			break;

		case MotionEvent.ACTION_MOVE:
			LLog.d("--onInterceptTouchEvent ACTION_MOVE--");
			mOnClick = false;
			mVelocityTracker.computeCurrentVelocity(1000,ViewConfiguration.getMaximumFlingVelocity());
			if (mScrollState == SCROLL_STATE_ALLOW && Math.abs(mVelocityTracker.getXVelocity()) > 200) {
				LLog.d("--onInterceptTouchEvent ACTION_MOVE return true--");
				return true;
			}
			break;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			LLog.d("--onTouchEvent ACTION_DOWN--");
			mTouchState = mScroller.isFinished() ? TOUCH_STATE_RESTART : TOUCH_STATE_SCROLLING;
			if (mTouchState==TOUCH_STATE_SCROLLING) {
				LLog.d("--onTouchEvent ACTION_DOWN return false--");
				return false;
			}
			break;

		case MotionEvent.ACTION_MOVE:
			LLog.d("--onTouchEvent ACTION_MOVE--");
			mVelocityTracker.computeCurrentVelocity(1000,ViewConfiguration.getMaximumFlingVelocity());
