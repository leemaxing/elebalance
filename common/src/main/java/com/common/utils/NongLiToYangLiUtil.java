...
public class NongLiToYangLiUtil {
	...
	public static String sCalendarLundarToSolar(String date) {
		String[] s = date.split("_");
		int iYear = Integer.parseInt(s[0]);
		int iMonth = Integer.parseInt(s[1]);
		int iDay = Integer.parseInt(s[2]);
...
class SolarDate extends MyDate {
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = NongLiToYangLiUtil.iGetSYearMonthDays(iYear, iMonth);
		if (iDay > iMonthDays) {
			return iMonthDays;
		} else if (iDay < 1) {
			return 1;
	...
	public Week toWeek() {
		...
		for (int i = 1901; i < iYear; i++) {
			if (NongLiToYangLiUtil.bIsSolarLeapYear(i))
				iOffsetDays += 366;
			else
				iOffsetDays += 365;
		}
		iOffsetDays += NongLiToYangLiUtil.iGetSNewYearOffsetDays(iYear, iMonth,
				iDay);
	...
	public LunarDate toLunarDate() {
		...
		LunarDate ld;
		iDate = Integer.parseInt(NongLiToYangLiUtil.sCalendarSolarToLundar(
...
class LunarDate extends MyDate {
	...
	private static int checkMonth(int iYear, int iMonth) {
		if ((iMonth > 12)
				&& (iMonth == NongLiToYangLiUtil.iGetLLeapMonth(iYear) + 12)) {
			return iMonth;
		} else if (iMonth > 12) {
			return 12;
		} else if (iMonth < 1) {
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = NongLiToYangLiUtil.iGetLMonthDays(iYear, iMonth);
		if (iDay > iMonthDays) {
			return iMonthDays;
		} else if (iDay < 1) {
			return 1;
	...
	public CnWeek toWeek() {
		int iOffsetDays = 0;
		for (int i = 1901; i < iYear; i++)
			iOffsetDays += NongLiToYangLiUtil.iGetLYearDays(i);
		iOffsetDays += NongLiToYangLiUtil.iGetLNewYearOffsetDays(iYear, iMonth,
				iDay);
	...
	public SolarDate toSolarDate() {
		int iYear, iMonth, iDay, iDate;
		SolarDate sd;
		iDate = Integer.parseInt(NongLiToYangLiUtil
				.sCalendarLundarToSolar(this.iYear + "-" + this.iMonth + "-"
						+ this.iDay));
		iYear = iDate / 10000;
		iMonth = iDate % 10000 / 100;
		iDay = iDate % 100;
