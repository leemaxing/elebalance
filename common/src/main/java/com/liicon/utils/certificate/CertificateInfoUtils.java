package com.liicon.utils.certificate;
...
import com.liicon.utils.encrypt.MD5Digest;
...
public class CertificateInfoUtils {
	 ...
	 * @return
	 */
	public static CertificateInfo getCertificateInfo(Context context, String packageName) {
		// 获取证书对象
		Signature signatures = CertificateInfoUtils.getSignature(context, packageName);
		if (signatures == null) {
			return null;
		}
		
		X509Certificate certificate = CertificateInfoUtils.getCertificate(signatures);
		if (certificate == null) {
			return null;
		}
		
		// 创建证书信息实体类
		CertificateInfo signInfo = new CertificateInfo();
	 ...
	 * @return ture:有效
	 */
	public static boolean verifyCertificateByPublicKey(Context context, String packageName, String publicKey) {
		if (TextUtils.isEmpty(publicKey)) {
			throw new NullPointerException("'PublicKey' cannot be empty.");
		}
		
		CertificateInfo signInfo = getCertificateInfo(context, packageName);
	 ...
	 * @return ture:有效
	 */
	public static boolean verifySignatureDigest(Context context, String packageName, String digest) {
		if (TextUtils.isEmpty(digest)) {
			throw new NullPointerException("'Digest' cannot be empty.");
		}
		
		if (digest.equals(getSignatureDigest(context, packageName))) {
			return true;
		}
		return false;
	}
