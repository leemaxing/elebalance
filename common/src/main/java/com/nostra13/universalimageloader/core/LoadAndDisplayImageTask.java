...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	public void run() {
		...
		LLog.d(LOG_START_DISPLAY_IMAGE_TASK, memoryCacheKey);
		if (loadFromUriLock.isLocked()) {
		    LLog.d(LOG_WAITING_FOR_IMAGE_LOADED, memoryCacheKey);
		}
		...
		try {
			...
			if (bmp == null || bmp.isRecycled()) {
				...
				if (options.shouldPreProcess()) {
				    LLog.d(LOG_PREPROCESS_IMAGE, memoryCacheKey);
					bmp = options.getPreProcessor().process(bmp);
					if (bmp == null) {
						LLog.e(ERROR_PRE_PROCESSOR_NULL, memoryCacheKey);
					}
				}

				if (bmp != null && options.isCacheInMemory()) {
					LLog.d(LOG_CACHE_IMAGE_IN_MEMORY, memoryCacheKey);
					configuration.memoryCache.put(memoryCacheKey, bmp);
				}
			} else {
				loadedFrom = LoadedFrom.MEMORY_CACHE;
				LLog.d(LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING, memoryCacheKey);
			}

			if (bmp != null && options.shouldPostProcess()) {
				LLog.d(LOG_POSTPROCESS_IMAGE, memoryCacheKey);
				bmp = options.getPostProcessor().process(bmp);
				if (bmp == null) {
					LLog.e(ERROR_POST_PROCESSOR_NULL, memoryCacheKey);
				}
			}
			checkTaskNotActual();
			checkTaskInterrupted();
	...
	private boolean waitIfPaused() {
		...
		if (pause.get()) {
			synchronized (engine.getPauseLock()) {
				if (pause.get()) {
					LLog.d(LOG_WAITING_FOR_RESUME, memoryCacheKey);
					try {
						engine.getPauseLock().wait();
					} catch (InterruptedException e) {
						LLog.e(LOG_TASK_INTERRUPTED, memoryCacheKey);
						return true;
					}
					LLog.d(LOG_RESUME_AFTER_PAUSE, memoryCacheKey);
				}
			}
		}
	...
	private boolean delayIfNeed() {
		if (options.shouldDelayBeforeLoading()) {
			LLog.d(LOG_DELAY_BEFORE_LOADING, options.getDelayBeforeLoading(), memoryCacheKey);
			try {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if (imageFile != null && imageFile.exists()) {
				LLog.d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, memoryCacheKey);
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				LLog.d(LOG_LOAD_IMAGE_FROM_NETWORK, memoryCacheKey);
	...
	private boolean tryCacheImageOnDisk() throws TaskCancelledException {
		LLog.d(LOG_CACHE_IMAGE_ON_DISK, memoryCacheKey);
		...
		try {
			...
			if (loaded) {
				...
				if (width > 0 || height > 0) {
					LLog.d(LOG_RESIZE_CACHED_IMAGE_FILE, memoryCacheKey);
	...
	private boolean downloadImage() throws IOException {
		...
		if (is == null) {
			LLog.e(ERROR_NO_IMAGE_STREAM, memoryCacheKey);
			return false;
		} else {
			try {
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		if (targetFile != null && targetFile.exists()) {
			...
			if (bmp != null && configuration.processorForDiskCache != null) {
				LLog.d(LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK, memoryCacheKey);
				bmp = configuration.processorForDiskCache.process(bmp);
				if (bmp == null) {
					LLog.e(ERROR_PROCESSOR_FOR_DISK_CACHE_NULL, memoryCacheKey);
				}
			}
			if (bmp != null) {
	...
	private boolean isViewCollected() {
		if (imageAware.isCollected()) {
			LLog.d(LOG_TASK_CANCELLED_IMAGEAWARE_COLLECTED, memoryCacheKey);
			return true;
		}
		return false;
	}
	...
	private boolean isViewReused() {
		...
		if (imageAwareWasReused) {
			LLog.d(LOG_TASK_CANCELLED_IMAGEAWARE_REUSED, memoryCacheKey);
			return true;
		}
		return false;
	}
	...
	private boolean isTaskInterrupted() {
		if (Thread.interrupted()) {
			LLog.d(LOG_TASK_INTERRUPTED, memoryCacheKey);
			return true;
		}
		return false;
	}
