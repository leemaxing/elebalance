...
public class BaseImageDecoder implements ImageDecoder {
	...
	public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
		...
		if (imageStream == null) {
			LLog.e(ERROR_NO_IMAGE_STREAM, decodingInfo.getImageKey());
			return null;
		}
		try {
		...
		if (decodedBitmap == null) {
			LLog.e(ERROR_CANT_DECODE_IMAGE, decodingInfo.getImageKey());
		} else {
	...
	protected ExifInfo defineExifOrientation(String imageUri) {
		...
		try {
			...
			LLog.i("Can't read EXIF tags from file [%s]", imageUri);
		}
	...
	protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
		...
		if (scale > 1) {
			LLog.d(LOG_SUBSAMPLE_IMAGE, imageSize, imageSize.scaleDown(scale), scale, decodingInfo.getImageKey());
		}
	...
	protected Bitmap considerExactScaleAndOrientatiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo,
		...
		if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
			...
			if (Float.compare(scale, 1f) != 0) {
				...
				{
					LLog.d(LOG_SCALE_IMAGE, srcSize, srcSize.scale(scale), scale, decodingInfo.getImageKey());
				}
			}
		}
		// Flip bitmap if need
		if (flipHorizontal) {
			...
			LLog.d(LOG_FLIP_IMAGE, decodingInfo.getImageKey());
		}
		// Rotate bitmap if need
		if (rotation != 0) {
			m.postRotate(rotation);

			LLog.d(LOG_ROTATE_IMAGE, rotation, decodingInfo.getImageKey());
		}
