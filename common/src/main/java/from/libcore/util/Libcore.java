...
import from.java.nio.charset.Charsets;
import from.org.eclipse.jetty.npn.NextProtoNego;
...
public class Libcore {
	public static void makeTlsTolerant(SSLSocket socket, String socketHost,
			boolean tlsTolerant) {
		if (!tlsTolerant) {
		...
		try {
			Class<?> openSslSocketClass = Class
					.forName("org.apache.harmony.xnet.provider.jsse.OpenSSLSocketImpl");
			if (openSslSocketClass.isInstance(socket)) {
				openSslSocketClass.getMethod("setEnabledCompressionMethods",
						String[].class).invoke(socket,
						new Object[] { new String[] { "ZLIB" } });
				openSslSocketClass.getMethod("setUseSessionTickets",
						boolean.class).invoke(socket, true);
	...
	public static byte[] getNpnSelectedProtocol(SSLSocket socket) {
		...
		try {
			Class<?> openSslSocketClass = Class
					.forName("org.apache.harmony.xnet.provider.jsse.OpenSSLSocketImpl");
			return (byte[]) openSslSocketClass.getMethod(
		...
		// Next try OpenJDK.
		JettyNpnProvider provider = (JettyNpnProvider) NextProtoNego
				.get(socket);
		if (!provider.unsupported && provider.selected == null) {
			throw new IllegalStateException(
					"No callback received. Is NPN configured properly?");
		}
		return provider.unsupported ? null : StringUtils.getBytes(provider.selected, Charsets.US_ASCII);
	}
	...
	public static void setNpnProtocols(SSLSocket socket, byte[] npnProtocols) {
		...
		for (int i = 0; i < npnProtocols.length;) {
			...
			strings.add(StringUtils.newString(npnProtocols, i, length, Charsets.US_ASCII));
			i += length;
		}
	...
	private static class JettyNpnProvider implements
			NextProtoNego.ClientProvider, NextProtoNego.ServerProvider {
		...
		@Override
		public void protocolSelected(String selected) {
	...
	public static void pokeInt(byte[] dst, int offset, int value,
			ByteOrder order) {
