...
public class BarrierExecutor {
    ...
    private ArrayList<KJTaskExecutor<?, ?, ?>> taskList = new ArrayList<KJTaskExecutor<?, ?, ?>>();
     ...
     * @return
     */
    public BarrierExecutor put(KJTaskExecutor<?, ?, ?> task) {
        if (task != null) {
            taskList.add(task);
        }
        return this;
    ...
    public void start(final KJTaskExecutor<?, ?, ?> finishOnUiTask) {
        if (isRunning) {
    ...
    private void startInternal(final CountDownLatch latch) {
        for (KJTaskExecutor<?, ?, ?> each : taskList) {
            each.setFinishedListener(new KJTaskExecutor.FinishedListener() {
                @Override
