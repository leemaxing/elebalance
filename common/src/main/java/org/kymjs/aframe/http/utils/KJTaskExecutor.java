...
public abstract class KJTaskExecutor<Params, Progress, Result> {
     ...
     * <b> 优化： </b> 不限制并发总线程数!让任务总能得到执行，且高效执行少量(<=活跃线程数)的异步任务。<br>
     ...
     * 用于瞬间大量并发的场景，比如，假设用户拖动{@link android.widget.ListView}
    ...
    private static class SmartSerialExecutor implements Executor {
        /**
         * 这里使用{@link ArrayDequeCompat}作为栈比{@link java.util.Stack}性能高
         */
